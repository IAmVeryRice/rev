...
{
  /* your code goes here.  */
    /* creates a source and sink process, the source continually forks children 
    one for every move, providing a processor is available. The sink collects the result and returns the best move
    */
    int pid = fork();
    if (pid == 0)
    {
        for (i = 0; i < noOfMoves; i++)
        {
            ...
            if (fork() == 0)
            {
                ...
                move_score = alphaBeta(l[i], c, u, noPlies, o, minscore, maxscore); // could take 10sof seconds
                /*use alphaBeta to search move i*/


                //pass move_score, i, positions_explored back via mailbox
                mailbox_send(barrier, move_score, i, positions_explored);
                multiprocessor_signal(processorAvailable); //this core is not available
                exit(0);
            }
        }
    }
    ...
    else
    {
        ...
        for (i=0; i < noOfMoves; i++)
        {
            ...
            mailbox_rec(barrier, &move_score, &move_index, &positions_explored); //first parameter is mailbox, second is the result, third is move index which is "i", fourth is different board positions its evaluated
		      ...
		      BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
  ...
  for (i=0; i < noOfMoves; i++) //keeps running until the index hits the number of moves
    {
      //child->
      move_score = alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore); // could take 10sof seconds, uses move alphabeta to search move i

      //parent->
      if (move_score > best) //once its gets results back, runs this code
	{
	  best = move_score; //sets best to value of move_score
	  *move = l[i]; //sets pointer move to value of l index in the for loop
	}
    }
